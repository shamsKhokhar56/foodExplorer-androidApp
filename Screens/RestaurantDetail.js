...
const RestaurantDetail = ({ navigation, route }) => {
	...
	const postHandler = async () => {
		...
		return await firebase
			...
			.then(() =>
				Alert.alert("Review posted", "Review has been posted succesfully")
			);
	};
			...
			<ScrollView keyboardShouldPersistTaps={"always"}>
				...
				<View style={styles.restaurantDetails}>
					...
					<View style={styles.ratingStyles}>
						<Text style={styles.textStyles}> {rating.slice(0, -5)} </Text>
				...
				<View style={styles.buttonView}>
					...
					<TouchableHighlight
						...
						activeOpacity={0.4}
						style={
							showReview ? styles.buttonStylesTrue : styles.buttonStylesFalse
						}
				...
				{loading ? (
					...
					<View>
						<LanguageText
							styles={[
								styles.textStyles,
								{ borderBottomColor: "grey", borderTopWidth: 2, fontSize: 28 },
							]}
							value={"foodItems"}
						/>
						{foodData.map((item) => {
							if (item.restaurantId === restId && !item.foodName.includes('Deal')) {
								if (
					...
					<View>
						...
						<Text>
							<Text style={styles.timeTextStyles}>
								<LanguageText styles={styles.timeTextStyles} value={"timing"} />
								<Text>
									: {openTime} - {closeTime}
								</Text>
							</Text>
